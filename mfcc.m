...
function [mfcc_coeff] = mfcc( filename )

	% read in the file
	[y, Fs] = audioread(filename);
	
	% pre-emphasis step to emphasize higher freq
	x = y(:, 1); % pick one channel as it is a stereo recording 
	new_y(1) = x(1);
	for i=2:numel(x)
		new_y(i) = x(i) - 0.95 * x(i-1); 
	end

	% framing the signal to prevent spectral leakage
	samples10ms = Fs / 100;
	%samples10ms = Fs / 5;
	numWindowsComputed = (numel(new_y)/samples10ms) - 3;
	for i=0:numWindowsComputed
		% pick out 30 ms of samples
		currFrame = new_y((1 + i*samples10ms) : (3*samples10ms + i*samples10ms));
		...
		mfcc_curr_coeff = dct(log_spectral);

		% add the envelopes into this
		%envelope_coeff = envelope(currFrame_hamming);
		%mfcc_curr_coeff = [mfcc_curr_coeff envelope_coeff];

		% and store these into the result matrix
		if exist('mfcc_coeff')
			mfcc_coeff = [mfcc_coeff; mfcc_curr_coeff];
		else 
			mfcc_coeff = mfcc_curr_coeff;
		end
	end % for numWindowsComputed
end % mfcc
